...
func summon_monster():
	...
	new_monster.set_progress(first_path_segment.curve.get_closest_offset(summoning_portal.position))
...
func setSummoningPortal():
	if SummoningState.current_state == SummoningState.summoning_states.CHOOSING_LOCATION:
		...
		var closest_point_to_mouse = first_path_segment.curve.get_closest_point(mouse_position)
